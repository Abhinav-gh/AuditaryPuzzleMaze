...
export function SimonPuzzle({ audioManager, onSolve, onSkip, onRestart }) {
  ...
  const speakBriefAndThen = useCallback((text, onDone) => {
  ...
  const playSequence = useCallback(
    (seq, options = {}) => {
      const { announceTurn = true } = options;
      clearAll();
      timersRef.current = [];
      ...
      const doneId = setTimeout(() => {
        ...
        if (announceTurn) {
          speakBriefAndThen("Your turn.", beginInput);
        } else {
          beginInput();
        }
      }, delay + 300);
    ...
    [audioManager, speakBriefAndThen],
  );
    ...
    const onKeyDown = (e) => {
      if (e.code === "Enter") {
        ...
        if (phaseRef.current === PHASE.EXPLORE) {
          speakBriefAndThen("Starting Simon. Listen carefully.", () => {
            playSequence(seqRef.current, { announceTurn: true });
          });
        }
        return;
      ...
      if (e.key === "r" || e.key === "R") {
        window.speechSynthesis.cancel();
        playSequence(seqRef.current, { announceTurn: false });
        return;
      }
      if (e.key === "i" || e.key === "I") {
        const msg =
          phaseRef.current === PHASE.EXPLORE
  ...
  const handleRestart = () => {
    ...
    phaseRef.current = PHASE.EXPLORE;
    const utter = new SpeechSynthesisUtterance(
      "Puzzle restarted. Explore the arrow keys, then press Enter to begin.",
    );
    window.speechSynthesis.speak(utter);
  };
      ...
      <div className="wordle-panel simon-panel">
        <div className="wordle-header">
          ...
          <p className="wordle-subtitle">
            {phase === PHASE.EXPLORE
              ? "Learn the sounds, then press Enter to start"
              : "Repeat the arrow-key note sequence"}
          </p>
        </div>
        ...
        <p className="rhythm-phase-label" aria-live="polite">
          {phase === PHASE.EXPLORE &&
            (allExplored
              ? "✅ All explored — press Enter to begin!"
              : `Explore: ${ARROWS.filter((k) => exploredKeys.has(k)).length}/4 keys heard`)}
          {phase === PHASE.PLAYBACK && "🔊 Listen carefully…"}
          {phase === PHASE.INPUT &&
            `↑↓←→ Your turn — step ${userInput.length + 1}/${sequence.length}`}
          {phase === PHASE.RESULT &&
            (result === "correct"
              ? "🎉 Correct!"
              : result === "wrong"
        ...
        <div className="simon-dpad">
          {ARROW_INFO.map(({ key, label, pos, desc }) => {
            ...
            return (
              <button
                ...
                onMouseDown={() => {
                  ...
                  if (phase === PHASE.EXPLORE)
                    setExploredKeys((prev) => new Set([...prev, key]));
                }}
                onMouseUp={() => {
                  setActiveKey(null);
              ...
              >
                {label}
                {phase === PHASE.EXPLORE && explored && (
                  <span className="explore-check">✓</span>
                )}
              </button>
            );
          })}
          <div className="dpad-center">
            {phase === PHASE.EXPLORE && (
              <button
                className="dpad-start-btn"
                onClick={() => playSequence(seqRef.current)}
                title="Start puzzle (Enter)"
              >
                ▶
              </button>
            )}
        ...
        {phase === PHASE.EXPLORE && (
          <div className="simon-explore-legend">
            {ARROW_INFO.map(({ key, label, desc }) => (
              <div
                key={key}
                className={`explore-item ${exploredKeys.has(key) ? "explored" : ""}`}
              >
        ...
        <div className="puzzle-shortcuts">
          {phase === PHASE.EXPLORE ? (
            <>
              <span>
                <kbd>↑↓←→</kbd> Explore notes
              </span>
              <span>
                <kbd>Enter</kbd> Start puzzle
              </span>
            </>
          ) : (
            <>
              <span>
                <kbd>↑↓←→</kbd> Input (on release)
              </span>
              <span>
                <kbd>R</kbd> Replay
              </span>
