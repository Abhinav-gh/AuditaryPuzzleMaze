...
const DIR_DELTA = { north:{dx:0,dy:-1}, south:{dx:0,dy:1}, east:{dx:1,dy:0}, west:{dx:-1,dy:0} };
...
function panOf(dir) { return dir === 'east' ? 0.8 : dir === 'west' ? -0.8 : 0; }
...
export default function App() {
  ...
  const checkDanger = useCallback((lv, x, y) => {
    ...
    if (dangers.length > 0) {
      ...
      speak(`Warning! Danger to the ${dirs}. Stay alert!`, { priority: true });
      return dangers;
    }
    return [];
  }, [addLog, speak]);
  ...
  useEffect(() => {
    if (screen !== 'playing' || !PUZZLE_PHASES.concat('playing').includes(phase)) return;
    ...
    const interval = setInterval(() => {
      ...
      if (!cur) return;
      if (!last) { mousePosRef.current = { ...cur }; return; }
      ...
      let cellType;
      if (tx < 0 || tx >= lv.cols || ty < 0 || ty >= lv.rows) cellType = CELL.WALL;
  ...
  const startLevel = async (lv) => {
    ...
    setScreen('playing'); setPhase('playing'); phaseRef.current = 'playing';
    setPos(lv.start); prevPosRef.current = lv.start; posRef.current = lv.start;
    setMoves(0);
    setVisitedCells(new Set([`${lv.start.x},${lv.start.y}`]));
    setSolvedPuzzles({}); setNearDanger([]); setLog([]);
    setPuzzleKey(0);
  ...
  const speakInstructions = useCallback(() => {
    window.speechSynthesis?.cancel();
    speak(buildInstructions(phaseRef.current, levelRef.current?.id ?? 1), { priority: true });
  }, [speak]);
  ...
  useEffect(() => {
    ...
    const handleKey = (e) => {
      ...
      if (e.key === 'p' || e.key === 'P') {
        ...
        speak(`Exit is to the ${ed.replace('-', ' ')}. Distance ${d.toFixed(1)} cells.`);
        return;
      }
      ...
      if (ct === CELL.EXIT) {
        ...
        setScreen('won');
        addLog(`🏆 EXIT REACHED!`);
        speak(`Congratulations! You escaped the maze in ${moves + 1} moves! Press Enter to play again, L for level select, or hold Escape for the main menu.`, { priority: true });
        return;
      }

      if (CELL_IS_PUZZLE(ct) && !solvedPuzzles[`${nx},${ny}`]) {
        audioRef.current.playPuzzleFound();
        const phaseName  = { [CELL.WORDLE]:'wordle', [CELL.CHORD]:'chord', [CELL.RHYTHM]:'rhythm', [CELL.SIMON]:'simon' }[ct];
        const kindLabel  = { [CELL.WORDLE]:'Word',   [CELL.CHORD]:'Chord', [CELL.RHYTHM]:'Rhythm', [CELL.SIMON]:'Simon Says' }[ct];
        addLog(`🔐 ${kindLabel} Puzzle at (${nx},${ny})`);
        speak(`${kindLabel} puzzle found! Press I for instructions.`, { priority: true });
  ...
  const exitDistance = (isInGame && lv) ? dist(pos.x, pos.y, lv.exit.x, lv.exit.y).toFixed(1) : '—';
      ...
      <div className="maze-grid" style={{ '--cols': lv.cols }}>
        {lv.maze.flatMap((row, ry) =>
          row.map((cell, cx) => {
            ...
            return (
              <div key={`${cx}-${ry}`} className={cls}
                title={isDanger ? 'DANGER' : isExit ? 'Exit' : {
                  [CELL.WORDLE]:'Word Puzzle', [CELL.CHORD]:'Chord Puzzle',
                  [CELL.RHYTHM]:'Rhythm Puzzle', [CELL.SIMON]:'Simon Puzzle',
                  [CELL.WALL]:'Wall',
                }[cell] ?? 'Path'}>
  ...
  return (
    <div className="app">
      ...
      {phase === 'wordle' && <AudioWordle key={`w-${puzzleKey}`} {...puzzleProps} />}
      {phase === 'chord'  && <ChordPuzzle key={`c-${puzzleKey}`} {...puzzleProps} />}
      {phase === 'rhythm' && <RhythmPuzzle key={`r-${puzzleKey}`} {...puzzleProps} />}
      {phase === 'simon'  && <SimonPuzzle  key={`s-${puzzleKey}`} {...puzzleProps} />}
      ...
      <main className="main-layout">
        ...
        {screen === 'splash' && (
          <div className="splash-card">
            ...
            <p className="splash-desc">An audio-first exploration game. Navigate mazes by sound, solve music puzzles, avoid lurking dangers.</p>
            <p className="splash-tip">🎧 Use headphones for the best experience</p>
            <button className="start-btn" id="choose-level-btn" onClick={() => {
        ...
        {screen === 'level-select' && (
          <div className="level-select">
            ...
            <div className="level-cards">
              {LEVELS.map((lv, i) => (
                <button key={lv.id} className="level-card" id={`level-${lv.id}-btn`}
                  onClick={async () => { audioRef.current = new AudioManager(); await startLevel(lv); }}>
            ...
            <section className="info-panel">
              <div className="status-panel">
                ...
                <div className="stats-row">
                  <div className="stat-card"><span className="stat-label">Position</span><span className="stat-value">({pos.x},{pos.y})</span></div>
                  <div className="stat-card"><span className="stat-label">Moves</span><span className="stat-value">{moves}</span></div>
                  <div className="stat-card"><span className="stat-label">Exit dist.</span><span className="stat-value">{exitDistance}</span></div>
                </div>
                <div className="open-paths-card">
                  <div><span className="stat-label">Open paths</span><span className="open-dirs">{openDirs}</span></div>
                  <div><span className="stat-label">Exit →</span><span className="open-dirs" style={{color:'#22c55e'}}>{exitDirection}</span></div>
                </div>
                {nearDanger.length > 0 && (
                  <div className="danger-warning" role="alert">
                    🦖 Danger to the <strong>{nearDanger.map(d=>d.dir).join(' & ')}</strong>!
                  </div>
                )}
...
function WinScreen({ lv, moves, resetToMenu, startLevel, audioRef, setScreen, setPhase }) {
  useEffect(() => {
    const h = (e) => {
      if (e.key === 'Enter') {
        audioRef.current = new AudioManager(); startLevel(lv);
      }
      if (e.key === 'l' || e.key === 'L') {
  ...
  return (
    <div className="win-card-full">
      ...
      <p className="splash-tip small" style={{marginTop: '1rem'}}>
        Press <kbd>Enter</kbd> to Play Again · <kbd>L</kbd> for Level Select · Hold <kbd>Esc</kbd> for Menu
      </p>
      <div className="win-actions">
        <button className="start-btn" id="play-again-btn"
          onClick={() => { audioRef.current = new AudioManager(); startLevel(lv); }}>
          🔄 Play Again
        </button>
